...
import { Check, ChevronRight, Camera, Mail } from 'lucide-react';
import { createClientEventAction, type CreateClientEventState } from './actions';
...
const STEPS = ['Event', 'Services', 'Review'] as const;
...
function ServiceIcon({ slug }: { slug: string }) {
  ...
  if (slug === 'web-invitation') return <Mail className="w-5 h-5" />;
...
const initialState: CreateClientEventState = {};

export function CreateEventWizard({ client, services }: WizardProps) {
  ...
  // Server action
  const boundAction = createClientEventAction.bind(null, client.id);
        ...
        <div>
          ...
          <div className="mb-6 flex items-center gap-3 px-4 py-3 rounded-lg bg-gray-50 border border-gray-200">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-gray-400 font-medium uppercase tracking-wider mb-0.5">Client</p>
              <p className="text-sm font-semibold text-gray-900 truncate">{client.name}</p>
              <p className="text-xs font-mono text-gray-400">{client.client_code}</p>
            </div>
          </div>
