...
export function ResetPinButton({ eventId }: { eventId: string }) {
  const [isOpen, setIsOpen] = useState(false);
  const [isPending, startTransition] = useTransition();

  function handleConfirm() {
    startTransition(async () => {
      await resetPinAction(eventId);
      setIsOpen(false);
    });
  }
  ...
  return (
    <>
      <button 
        ...
        className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-gray-900 transition-colors"
      >
        Reset PIN
      </button>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 backdrop-blur-sm">
          <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl text-left">
            <h2 id="reset-pin-modal-title" className="text-lg font-bold text-gray-900">Reset PIN?</h2>
            <div className="mt-4 text-sm text-gray-500 space-y-1">
              <p>The current PIN will stop working.</p>
              <p>Existing uploaded photos will not be affected.</p>
              <p>New contributors must use the new PIN.</p>
            </div>
