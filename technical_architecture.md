...
> *Status: MVP Architecture Definition (Based on PRD v1.3 FINAL)*

---

## 1. High-level System Architecture

The AlbumCerita MVP is a serverless, managed-service platform optimized for mobile-first photo capture and real-time gallery updates.
...
Implementation of the PRD v1.3 schema with strict data typing.
...
CREATE TABLE events (
  ...
  max_contributors integer, -- NULL means unlimited
...
**`events` Table:**
- `SELECT`: Public. Only returns rows where `state IN ('draft', 'published')`. Expired/Archived are hidden from public queries.
...
- `INSERT` (Guest): Allowed via a PostgreSQL function or Server Action that performs the contributor limit check and `event.state = 'draft'` check atomically before writing.
...
8. **Realtime:** Supabase Realtime broadcasts new row to clients viewing the gallery/dashboard.
...
7. Supabase Realtime broadcasts state change.
...
None. This architecture aligns 100% with the locked constraints of `PRODUCT_REQUIREMENTS_v1.3_FINAL.md`. Ready for execution upon approval.
